...
function findInvalidCards(nestedArr){
    let arrInvalidNumbers = [];
    arrInvalidNumbers = nestedArr.filter(card => validateCred(card));
    return arrInvalidNumbers;
}
...
function idInvalidCardCompanies(arrInvalid){
    ...
    //element of the array is just a number
    if(!Array.isArray(arrInvalid[0])){
        switch(arrInvalid[0]){
            case 3: 
        ...
        //element of the array is an array (batch)
        for(let i = 0; i < arrInvalid.length; i++){
            switch (arrInvalid[i][0]){
                case 3: 
                    invalidCompaniesList.push("Amex (American Express)"); 
                break;
                case 4: 
         ...
         if(invalidCompaniesList.indexOf("Visa") != -1){
             invalidCompanies.push("Visa");
         }
         if(invalidCompaniesList.indexOf("Amex (American Express)") != -1){
             invalidCompanies.push("Amex (American Express)");
         }
         if(invalidCompaniesList.indexOf("Mastercard") != -1){
             invalidCompanies.push("Mastercard");
         }
         if(invalidCompaniesList.indexOf("Discover") != -1){
...
console.log(idInvalidCardCompanies(valid2));
...
function convertArray(str){
         let convertedStringToArray = [];
         for(let i = 0; i < str.length; i++){
             convertedStringToArray.push(str[i]);
     }
      return convertedStringToArray;
}

//test convertArray
console.log(convertArray('123456789'));
